...
static ngx_str_t ngx_h264_para_start = ngx_string("start");
static ngx_str_t ngx_h264_para_end = ngx_string("end");
static ngx_str_t ngx_h264_para_null = ngx_string("null");

typedef struct ngx_http_h264_param_s {
	ngx_str_t domain;
	ngx_str_t start;
	ngx_str_t end;
	ngx_str_t uri;//
...
}ngx_http_h264_param_t;

typedef struct {
    ngx_array_t                  *params;
    ngx_uint_t                   flag;
} ngx_http_h264_loc_conf_t;

static char* ngx_streaming(ngx_conf_t* cf, ngx_command_t* cmd, void* conf);

static char *
ngx_http_h264_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child);

static void *
ngx_http_h264_create_loc_conf(ngx_conf_t *cf);

static char *
ngx_http_h264_param_set_slot(ngx_conf_t *cf, ngx_command_t *cmd,void *conf);

static ngx_int_t
ngx_http_h264_get_param(ngx_http_request_t *r,ngx_http_h264_param_t *param);

static ngx_int_t
ngx_http_h264_get_start_end(ngx_http_request_t *r,ngx_http_h264_param_t *param
...
static ngx_command_t ngx_streaming_commands[] =
{
  ...
  {
#ifdef BUILDING_H264_STREAMING
    ngx_string("mp4_param"),
#endif
#ifdef BUILDING_SMOOTH_STREAMING
    ngx_string("ism_param"),
#endif
   NGX_HTTP_LOC_CONF|NGX_CONF_2MORE,
   ngx_http_h264_param_set_slot,
...
static ngx_http_module_t ngx_streaming_module_ctx =
{
  ...
  ngx_http_h264_create_loc_conf, /* create location configuration */
  ngx_http_h264_merge_loc_conf   /* merge location configuration */
};
...
static ngx_int_t ngx_streaming_handler(ngx_http_request_t *r)
{
  ...
  ngx_http_h264_param_t       h264param;
...
/*  if (r->zero_in_uri)
  ...
  */
  ...
  if(ngx_http_h264_get_param(r,&h264param)==NGX_OK){
	  ngx_http_h264_get_start_end(r,&h264param,options);
  }
...
ngx_http_h264_create_loc_conf(ngx_conf_t *cf)
{
    ngx_http_h264_loc_conf_t  *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_h264_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
ngx_http_h264_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_h264_loc_conf_t *prev = parent;
    ngx_http_h264_loc_conf_t *conf = child;
...
ngx_http_h264_param_set_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_uint_t              i;
    ngx_str_t               *value;
    ngx_http_h264_param_t    *pa;
    ngx_http_h264_loc_conf_t *hlcf = conf;
    ngx_int_t 				n;
    ...
    if (hlcf->params == NGX_CONF_UNSET_PTR) {
    	hlcf->params = ngx_array_create(cf->pool, 1,
                                         sizeof(ngx_http_h264_param_t));
        if (hlcf->params == NULL) {
            return NGX_CONF_ERROR;
        }
    }

    pa = ngx_array_push(hlcf->params);

    if (pa == NULL) {
    	return NGX_CONF_ERROR;
    }else{
    	ngx_memset(pa,0,sizeof(ngx_http_h264_param_t));
    	pa->start = ngx_h264_para_start;
    	pa->end = ngx_h264_para_end;
    }
...
ngx_http_h264_get_param(ngx_http_request_t *r,ngx_http_h264_param_t *param)
{
	...
	ngx_uint_t                 i;
	ngx_http_h264_param_t       **params;
	ngx_str_t 				   host,uri;
	ngx_http_h264_loc_conf_t   *hlcf;
	ngx_http_core_srv_conf_t  *cscf;

#ifdef BUILDING_H264_STREAMING
	hlcf = ngx_http_get_module_loc_conf(r, ngx_http_h264_streaming_module);
#endif
#ifdef BUILDING_SMOOTH_STREAMING
	hlcf = ngx_http_get_module_loc_conf(r, ngx_http_smooth_streaming_module);
#endif
    ...
    param->domain = ngx_h264_para_null;
    param->start = ngx_h264_para_null;
    param->end = ngx_h264_para_null;
    param->flag =0;
    param->specal_flag =0;
    param->uri = ngx_h264_para_null;

    if(hlcf->params){
    	params = hlcf->params->elts;
    	for(i=0;i< hlcf->params->nelts;i++) {
		...
		struct mp4_split_options_t* options)
{
	...
	if (ngx_http_arg(r, param->start.data , param->start.len, &value) == NGX_OK) {
		valz =ngx_palloc(r->pool,value.len+1);
		ngx_cpystrn((u_char *)valz,value.data,value.len);
