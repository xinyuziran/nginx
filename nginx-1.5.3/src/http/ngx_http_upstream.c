            ...
            ngx_http_file_cache_valid(u->conf->cache_valid, u->headers_in.status_n))
        {
            ...
            if (rc == NGX_OK) {
                ...
                if (valid) {
                	...
                	// Write cache
                	if (ngx_write_fd(fd, u->buffer.start, sizeof(ngx_http_file_cache_header_t)) < 0){
                		ngx_log_error(NGX_LOG_CRIT, r->connection->log, ngx_errno,
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if((r->cache) && (r->cache->file_cache)){
    	if(r->cache->file_cache->http_cache_mode == NGX_HTTP_CACHE_READ){
    		p->cacheable =0;
    	}
    }else{
    	if((u->conf) && (u->conf->store_mode==NGX_HTTP_CACHE_READ)){
    		p->cacheable =0;
    	}
    }
    ...
    ngx_table_elt_t *h, ngx_uint_t offset)
{
    ...
    *ph = h;
    ...
    //flashapp fix Expires and cache-control are all set,max-age was set for ignore Expires
    u->cacheable = 1;
    }
#endif
    ...
    ngx_uint_t offset)
{
    ...
    //flashapp for Expires cache many after cache-control
    u->cacheable = 1;
    }
#endif
