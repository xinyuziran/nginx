...
static ngx_int_t ngx_http_variable_sent_content_length(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
//add for flashapp
static ngx_int_t ngx_http_variable_sent_original_length(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_request_is_sdk(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
...
//add for flashapp
    { ngx_string("flashapp_original_length"), NULL,
      ngx_http_variable_sent_original_length, 0, 0, 0 },
    { ngx_string("flashapp_is_sdk"), NULL,
      ngx_http_variable_request_is_sdk, 0,
      NGX_HTTP_VAR_NOCACHEABLE, 0 },
...
//add for flashapp
static ngx_int_t
ngx_http_variable_sent_original_length(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    u_char  *p;
    ngx_str_t original1=ngx_string("sent_http_original_length");
    ngx_str_t original2=ngx_string("sent_http_x_original_content_length");
    if (ngx_http_variable_unknown_header_out(r, v, (uintptr_t)&original1)
                == NGX_OK)
    {
    	if(v->not_found==0){
    		return NGX_OK;
    ...
    if (ngx_http_variable_unknown_header_out(r, v, (uintptr_t)&original2)
                == NGX_OK)
    {
    	if(v->not_found==0){
    		return NGX_OK;
...
static ngx_int_t
ngx_http_variable_request_is_sdk(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
	u_char  *p;
	ngx_str_t original1=ngx_string("http_code");
	ngx_str_t original2=ngx_string("http_token");
	if (ngx_http_variable_unknown_header_in(r, v, (uintptr_t)&original1)
	     == NGX_OK)
	{
		if(v->not_found==0){
			return NGX_OK;
	...
	if (ngx_http_variable_unknown_header_in(r, v, (uintptr_t)&original2)
	     == NGX_OK)
	{
		if(v->not_found==0){
			return NGX_OK;
