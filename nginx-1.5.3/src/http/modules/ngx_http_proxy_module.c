...
/* ****************
 *
 * add by xiaoguo 2013-10-31
 *
 */
static ngx_int_t 
     ngx_http_proxy_store_file_status_variable(ngx_http_request_t *r,
     ngx_http_variable_value_t *v, uintptr_t data);
/* ****************
 *
 * end by xiaoguo 2013-10-31
 *
 */
...
static char *
ngx_http_proxy_set_store_mode_slot(ngx_conf_t *cf, ngx_command_t *cmd,
	void *conf);

static char *
ngx_http_proxy_store_file_expries(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
...
//add for flashapp
    { ngx_string("proxy_store_mode"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_proxy_set_store_mode_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },

	{ ngx_string("proxy_store_file_expries"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
	  ngx_http_proxy_store_file_expries,
	  NGX_HTTP_LOC_CONF_OFFSET,
	  0,
	  NULL },
...
static ngx_keyval_t  ngx_http_proxy_cache_headers[] = {
    ...
    { ngx_string("Expect"), ngx_string("") },
    { ngx_string("Upgrade"), ngx_string("") },
    // flashapp add for check modified
...
//add by xiaoguo 2013-10-31
	{ ngx_string("proxy_store_file_status"), NULL,
	  ngx_http_proxy_store_file_status_variable, 0,
...
/* ****************
 * add by xiaoguo 2013-10-31
 * 判断所请求中的store文件是否有效，进而决定是否从本地获取请求的文件；
 * 如果有效则$proxy_store_file_status为 1, 反之则为 0
 */
static ngx_int_t
ngx_http_proxy_store_file_status_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    ngx_http_proxy_loc_conf_t *plcf;
    plcf = ngx_http_get_module_loc_conf( r, ngx_http_proxy_module );
    allow_time = plcf->upstream.store_file_expries;
    if (ngx_http_map_uri_to_path(r, &path, &root, 0) == NULL) {
    	return NGX_ERROR;
    }
    /* ngx_http_map_uri_to_path() allocates memory for terminating '\0' */
    if( ngx_file_info( path.data, &ft ) == NGX_FILE_ERROR ){
    	return NGX_ERROR;
    }
    file_ctime = ngx_file_ctime( &ft ); //获取文件改变时间（文件属性也就是创建）--时间
    time( &loc_time );
    ...
    v->not_found = 0;
    if(allow_time >0){
    	if( loc_time  > ( file_ctime + allow_time ) )
    	v->data = (u_char *)"0";
    }else{
		v->data = (u_char *)"1";
    }
...
ngx_http_proxy_create_loc_conf(ngx_conf_t *cf)
{
    ...
    //add for flashapp
    conf->upstream.store_mode = NGX_CONF_UNSET_UINT;
    conf->upstream.store_file_expries = NGX_CONF_UNSET;
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    //add for flashapp
    ngx_conf_merge_uint_value(conf->upstream.store_mode,
                              prev->upstream.store_mode, NGX_HTTP_CACHE_NORMAL);

    ngx_conf_merge_value(conf->upstream.store_file_expries,
                                  prev->upstream.store_file_expries, 0);
...
//add for flashapp
static char *
ngx_http_proxy_set_store_mode_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_proxy_loc_conf_t *plcf = conf;
    ngx_str_t                  *value;
    ngx_uint_t                 i;
    ngx_uint_t                 cache_mode=NGX_HTTP_CACHE_NORMAL;

    value = cf->args->elts;

    for (i = 1; i < cf->args->nelts; i++) {
    	if(ngx_strncmp(value[i].data,"reader",6) == 0){
        	cache_mode =NGX_HTTP_CACHE_READ;
        }else if(ngx_strncmp(value[i].data,"writer",6) == 0){
        	cache_mode =NGX_HTTP_CACHE_WRITE;
        }else if(ngx_strncmp(value[i].data,"manager",7) == 0){
        	cache_mode =NGX_HTTP_CACHE_MANAGE;
        }else{
        	cache_mode =NGX_HTTP_CACHE_NORMAL;
        }
        continue;
    ...
    plcf->upstream.store_mode=cache_mode;
...
/*
 * add by xiaoguo 2013-10-30
 * 获取store_file_expries,从配置文件获取指定的有效时间
 */
static char *
ngx_http_proxy_store_file_expries(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_proxy_loc_conf_t *plcf = conf;
    ngx_str_t                  *value;

    if(plcf->upstream.store_file_expries >0){
    	return "is duplicate";
    }
    value = cf->args->elts;
    if(cf->args->nelts >1 ){
    	plcf->upstream.store_file_expries = ngx_parse_time(&value[1], 1);
    	if(plcf->upstream.store_file_expries!=(time_t)NGX_ERROR){
    		return NGX_CONF_OK;
    	}
    	ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
    	                       "invalid parameter \"%V\"", &value[1]);
