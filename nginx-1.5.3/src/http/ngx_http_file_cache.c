...
ngx_http_file_cache_update(ngx_http_request_t *r, ngx_temp_file_t *tf)
{
    ...
    c = r->cache;
// add for flashapp
    if((r->cache) && (r->cache->file_cache) &&
    		(c->file_cache->http_cache_mode == NGX_HTTP_CACHE_READ)){
    	return;
    }
...
ngx_http_file_cache_free(ngx_http_cache_t *c, ngx_temp_file_t *tf)
{
    ...
    if (c->temp_file) {
    	//add for flashapp
    	if(cache->http_cache_mode !=NGX_HTTP_CACHE_READ){
        ...
        if(cache->http_cache_mode!=NGX_HTTP_CACHE_READ){
        	...
        	*p = '\0';
        	...
        	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
           	...
           	//flashapp  for add some doamin folder
        	if(cache->http_cache_mode==NGX_HTTP_CACHE_NORMAL){
...
ngx_http_file_cache_set_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        //add for flashapp
        if (ngx_strncmp(value[i].data, "cache_mode=", 11) == 0) {
        	if((value[i].len >=17) && (ngx_strncmp(value[i].data + 11,"reader",6) == 0)){
        		cache_mode =NGX_HTTP_CACHE_READ;
        	}else if((value[i].len >=17) && (ngx_strncmp(value[i].data + 11,"writer",6) == 0)){
        		cache_mode =NGX_HTTP_CACHE_WRITE;
        	}else if((value[i].len >=18) && (ngx_strncmp(value[i].data + 11,"manager",7) == 0)){
    ...
    //add for flashapp
    cache->oldname.len = cache->path->name.len;
    cache->oldname.data = ngx_pstrdup(cf->pool, &(cache->path->name));
    ...
    //add for flashapp
    if((cache_mode == NGX_HTTP_CACHE_NORMAL) || (cache_mode == NGX_HTTP_CACHE_MANAGE)){
    ...
    //add for flashapp
    cache->http_cache_mode = cache_mode;
...
//add for flashapp
ngx_int_t
ngx_http_file_cache_path_fix(ngx_http_request_t *r)
{
    size_t      len;
    u_char     *p, *n;
    ngx_http_cache_t  *c;
    ...
    if(r->cache !=NULL){
    	c=r->cache;
    	if((c->file_cache !=NULL) && (c->file_cache->path!=NULL)){
    		cache=c->file_cache;
    		if(cache->http_cache_mode!=NGX_HTTP_CACHE_NORMAL){
    			if (r->headers_in.server.len) {
    				len = r->headers_in.server.len + cache->oldname.len;
    				p = r->headers_in.server.data;

    			} else {
    				cscf = ngx_http_get_module_srv_conf(r, ngx_http_core_module);
    				len = cscf->server_name.len + cache->oldname.len;
    				p = cscf->server_name.data;
    			}

    			n = ngx_pnalloc(r->pool, len + 2);
    			if (n == NULL) {
    				return NGX_ERROR;
    			}
    			ngx_cpystrn(n,cache->oldname.data,cache->oldname.len+1);
    			n[cache->oldname.len] = '/';
    			ngx_cpystrn(n+cache->oldname.len+1,p,len-cache->oldname.len+1);
    			cache->path->name.len = len+1;
    			cache->path->name.data = n;
    			return NGX_OK;
    		}
    	}
    }
